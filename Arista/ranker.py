def rank_results():
    ...
    print(f"[RANKER] OLD_IPS={len(old_lines)}")
    ...
    print(f"[RANKER] COMBINED_IPS={len(combined_lines)}")

    current_size_mb = get_file_size_mb(combined_lines)

    if current_size_mb > MAX_BEST_IPS_SIZE_MB:
        print(f"[RANKER] SIZE {current_size_mb:.2f}MB EXCEEDED {MAX_BEST_IPS_SIZE_MB}MB! REMOVING WORST TCP ENTRIES...")
        removed_count = 0
        while current_size_mb > MAX_BEST_IPS_SIZE_MB and len(combined_lines) > 100:
            removed = combined_lines.pop()
            removed_count += 1
            current_size_mb = get_file_size_mb(combined_lines)
        print(f"[RANKER] REMOVED {removed_count} IPS, NEW SIZE: {current_size_mb:.2f}MB")

    with open(BEST_FILE, "w", encoding="utf-8") as f:
        f.writelines(combined_lines)

    print(f"[RANKER] FINAL SIZE: {current_size_mb:.2f}MB, LINES: {len(combined_lines)}")
    print(f"[RANKER] RANKED={len(ranked)} DOMAINS={len(domains_set)} DOMAIN_IPS={len(domains_ips)}")
